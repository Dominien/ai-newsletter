...
**Erstellung einer Umgebungsvariablen**: Erstellen Sie eine Datei mit dem Namen `.env` im Stammverzeichnis Ihres Projekts. Diese Datei ist der ideale Ort, um sensible Informationen wie API-Schlüssel zu speichern, da sie nicht mit dem Quellcode Ihres Projekts geteilt wird.

2.

**Speichern des API-Schlüssels**: Fügen Sie in der `.env`-Datei den folgenden Eintrag hinzu:
